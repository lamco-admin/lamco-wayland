...
- **BREAKING**: `StreamInfo` gained a `mapping_id: Option<String>` field,
  ...
  >= 5). Correlates a stream to its EIS absolute-pointer/touch device for the
  same monitor, needed for correct input targeting in multi-monitor
  RemoteDesktop sessions. `None` on portal backends that don't provide it.
  Existing struct-literal construction of `StreamInfo` needs the new field
  added (`mapping_id: None` if unused).
