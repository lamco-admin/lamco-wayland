...
- **`SPA_PARAM_Tag` support: read the tags a producer publishes, and publish
  tags back.** Backport of 0.6.13. Tags are how a producer annotates a stream
  with information that is neither format nor buffer layout, and how a consumer
  answers. They arrive on the existing `param_changed` callback, so reading them
  needed no new subscription. GNOME's Mutter uses this on a virtual-monitor
  stream: it publishes the logical monitor's scale as `org.gnome.scale`, and
  reads `org.gnome.preferred-scale` back to decide which scale to apply. Nothing
  in this crate is GNOME-specific; the mechanism is PipeWire's own.
