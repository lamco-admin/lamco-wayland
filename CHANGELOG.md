...
### Added
- **lamco-pipewire 0.6.13:** `SPA_PARAM_Tag` read and write. Reads the tags a
  producer publishes on a stream (GNOME publishes the monitor scale as
  `org.gnome.scale`) and can publish tags back upstream, which is how a consumer
  states a preference such as `org.gnome.preferred-scale`. See lamco-pipewire's
  own changelog. `lamco-portal` is unaffected and stays at 0.4.5.

## [0.6.14] - 2026-08-30

### Fixed
- **lamco-pipewire 0.6.12:** the DMA-BUF mmap cache could serve a destroyed
  ...
  unaffected and stays at 0.4.5.

### Added
- **lamco-pipewire 0.6.12:** `BufferMeta::mappable` and `BufferMeta::chunk_empty`,
...
### Added
- **lamco-pipewire 0.6.11:** `PipeWireThreadManager::corrupted_buffer_count()`,
  ...
  The per-buffer warning for those is now rate limited. See lamco-pipewire's
  own changelog. `lamco-portal` is unaffected and stays at 0.4.4.
...
### Changed
- **lamco-pipewire 0.6.10:** stream timing now goes through pipewire-rs's own
