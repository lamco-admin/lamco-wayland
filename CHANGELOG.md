...
### Added
- **BREAKING**: `lamco-portal` 0.4.4's `StreamInfo::mapping_id` field
...
### Fixed
- **lamco-pipewire 0.5.8: disambiguate `modifier=0x0` in the stream
  ...
  present, instead of printing a bare `0x0`. Same fix as lamco-pipewire
  0.6.8 on the modern line. Requires the `v0_3_65` libspa feature
  (compile-time bindgen gate only: does not raise this line's low
  libpipewire>=0.3.33 floor). No API change.
- **lamco-wayland 0.5.8:** metacrate bump re-bundling lamco-pipewire 0.5.8.
...
### Fixed
- **lamco-pipewire 0.5.6: log the formats we offered when a stream enters
  ...
  locally. No API or behavioral change. Ported from the 0.6.x line
  unchanged — the negotiation code is identical between lines.
- **lamco-wayland 0.5.6:** metacrate bump re-bundling lamco-pipewire 0.5.6.

## [0.5.5] - 2026-07-23

### Fixed
- **lamco-pipewire 0.5.5: honor the requested audio capture rate and channel
...
- **lamco-wayland 0.5.5:** metacrate bump re-bundling lamco-pipewire 0.5.5.

## [0.5.4] - 2026-07-23

### Changed
- **lamco-pipewire 0.5.4: quiet the empty-frame capture path.** On some
...
- **lamco-wayland 0.5.4:** metacrate bump re-bundling lamco-pipewire 0.5.4.
